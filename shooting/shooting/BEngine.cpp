...
shared_ptr<BEngine> g_sEngine = nullptr; //전역변수로 언제든 참조할 수 있도록 엔진을 만듦
	...
	m_fDT(0.0f), m_fAcc(0.f), m_iCallCount(0),m_iFPS(0)
{
	...
	//현재 성능 카운터의 타이머 주파수만한 초당 카운트 수를 반환한다.
...
{
	g_sEngine = shared_from_this(); //전역변수에 자기자신을 공유할당
	Init(); //맵에서 초기화할 직업이 있다면 실행해서 초기화한다.
...
void BEngine::MainLoop()
{
	...
	if (m_fAcc >= 1.0f)
	{
		...
		m_fAcc = 0;
		m_iCallCount = 0;
		RenderText(m_iFPS, m_fDT); //윈도우 캡션창에 띄울것.
	}

	_Update(m_fDT);

	HDC hdc = GetDC(m_hWnd); //윈동 핸들로부터 DC를 얻음
	_Render(hdc, m_fDT); // 화면 그리기

	ReleaseDC(m_hWnd, hdc); //다 썻으면 반환
...
void BEngine::_Render(HDC hdc, float deltaTime)
{
	...
	GetClientRect(this->GetWindowHandle(), &winRect); //현재 윈도우 핸들을 통해서 윈도우의 크기를 정확하게 알아낸다.

	hMemDC = CreateCompatibleDC(hdc); //현재 HDC와 호환되는 메모리 pc를 하나 생성한다.
	bitmap = CreateCompatibleBitmap(hdc, winRect.right, winRect.bottom); //호환 비트맵 생성후 
	SelectObject(hMemDC, bitmap);
	FillRect(hMemDC, &winRect, (HBRUSH)COLOR_BACKGROUND); // 회색으로 사각형을 그려줌

	Render(hMemDC, deltaTime); //자식으로 이 메모리 DC를 넘겨준다. 자식은 실제론 이 메모리 DC에 그림을 덧대서 계속 그린다.

	//상속받은 녀석들이 bitmap개체와 memDC에 그린 것들을 전부 그렸다면 Bitblt를 이용해서 실제 화면에 출력하도록 보내준다.
